...
class MLPipeline:
    ...
    def get_dataset(self) -> None:
        self.dataset = dataset_factory.DatasetFactory(self.config.items.datasets).get(
            self.config.items.project.dataset
        )
    ...
    def evaluate_model(self) -> None:
        ...
        self.model.evaluate(
            self.dataset.df[self.config.items.project.features].loc[self.idx_test],
            self.dataset.df[self.config.items.project.target].loc[self.idx_test],
        )
...
if __name__ == "__main__":
    ...
    print(f"Pipeline run complete. See all artifacts in {pipeline.artifact_dir}")
